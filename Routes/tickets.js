...
router.post('/', auth, allowRoles('USER', 'MANAGER'), async (req, res) => {
...
router.patch('/:id/assign', auth, allowRoles('MANAGER', 'SUPPORT'), async (req, res) => {
  const { id } = req.params;
  const { assigneeId } = req.body;
  const user = await User.findById(assigneeId).populate('role');
  if (!user) return res.status(404).json({ message: 'User not found' });
  if (user.role.name === 'USER') return res.status(400).json({ message: 'Cannot assign to USER role' });
...
router.patch('/:id/status', auth, allowRoles('MANAGER', 'SUPPORT'), async (req, res) => {
  const { id } = req.params;
  const { status } = req.body;
  if (!['OPEN', 'IN_PROGRESS', 'RESOLVED', 'CLOSED'].includes(status)) return res.status(400).json({ message: 'Invalid status' });
  const ticket = await Ticket.findById(id);
  if (!ticket) return res.status(404).json({ message: 'Ticket not found' });
  if (!validStatusTransition(ticket.status, status)) return res.status(400).json({ message: 'Invalid transition' });
...
router.get('/:id', auth, async (req, res) => {
  ...
  if (!ticket) return res.status(404).json({ message: 'Ticket not found' });
  const role = req.user.role.name;
  if (role === 'MANAGER') return res.json(ticket);
  if (role === 'SUPPORT' && ticket.assigned_to && ticket.assigned_to._id.equals(req.user._id)) return res.json(ticket);
